...
10. Dataless (evicted) iCloud files fail Node reads and copies with EAGAIN instead of blocking to materialize, and right after a mass eviction the provider refuses every reader — `cat` included — for a transient window. The pipeline retries each such failure inside a bounded per-file budget (30 s): delayed re-reads, then a final materializing `cat`-equivalent read (sync-vault note reads), remove-and-recreate retries (publish copyfile) — and fails loudly only when EAGAIN outlasts the budget (issues #216, #229). This is a bounded wait, not lag machinery (rule 3); Keep Downloaded (rule 2) stays the primary posture.
