...
4. Budget one full re-ingest: the snapshot is stamped with the data repo root at write time (issue #95), so the first `wiki-ingest` after the rename reads a foreign-stamped snapshot, warns loudly, and falls back to a full run (~1 min/note; raise `--timeout` accordingly, e.g. 14400). Nothing is lost — the fallback is correct by design. Time the rename right after a topology rebuild, when the next run is a full run anyway, and the cost is zero.
