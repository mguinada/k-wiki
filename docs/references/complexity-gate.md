...
- **Nightly + manual dispatch (advisory):** the `mutation-nightly`
  (windowed schedule arm) and `mutation-merge` (dispatched full run)
  jobs run `npm run complexity:full`, upload `complexity-debt-report`
  ...
  runs were chunked into parallel jobs, issue #236; it moved again,
  to the windowed `mutation-nightly` job, when the chunked full run
  became dispatch-only, issue #261.)
