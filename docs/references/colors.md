...
is *uncolored messages*, and severity is detected at the sink
...
- `onProgress` messages are uncolored strings. Severity is detected at
  the sink (`createAgentProgressSink`, `colorizeProgress`), not at the
  call site.
- `createAgentProgressSink` takes the caller's `picocolors` tones and
  renders `WARNING`-containing messages `yellow`, all others `dim`.
- `colorizeProgress` (`src/sync/sync-vault.ts`) applies the same rule
  for its self-built sink, plus `bold` vault names.
