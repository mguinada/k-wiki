 ...
 * verb row and on a write-note verb not wired to its gate. The
 * table, its derived lists, and the bare help are builder
 * functions, not module-scope constants (issue #354): module-init
 * data is static to Stryker's per-test coverage — each of its
 * mutants re-runs the whole suite — while builder bodies run
 * inside covering tests.
 */
 ...
 *  tier, built at call time (issue #354: module-init data is
 *  mutation-static). A new launcher lands with its row in the same
 *  change (the drift guard in tests/cli/k-wiki.test.ts fails
 *  otherwise). */
export function verbTable(): readonly VerbSpec[] {
  return [
    {
      name: "query",
      klass: "read",
