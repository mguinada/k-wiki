...
2. Read the issue in full — goal, scope checklist, acceptance criteria, implementation notes, out-of-scope list, referenced docs. If the issue is not open, stop and say so. Note the issue's base-path impact statement (AGENTS.md "Simple stays simple"): the default answer is "nothing — default unchanged"; any other answer is written justification, and the PR body's base-path-impact line carries it — the reviewer checks the claim against the diff.
...
7. Read the referenced docs and the related code, using Codegraph (`explore`, `node`, `impact`) rather than blind search, and following the repository's own `AGENTS.md` instructions where they apply — when it carries an Architecture alignment block, its bounded-context ledger says which domain owns the change and what it must never touch.
...
12. Run the project's quality gates — typecheck, lint, full test suite, E2E tests where they exist — and fix failures; the work is not successful until the relevant checks pass. Then run the advisory checks the repository defines (in this repo: the dev-loop mutation run is optional — see AGENTS.md's mutation section; when run and it prints survivors, triage them per the repo's mutation-triage skill; they are advisory, never blocking — CI files survivors into the rolling issue).
