...
17. Open the PR, then push through the no-mistakes gate and drive it to an outcome. Run this whole step from the issue worktree's cwd (`$PWD`). The pane path requires Herdr — check `test "${HERDR_ENV:-}" = 1` first; if it fails, warn the user and drive the gate blocking in your own shell instead — push with the same `-o` options, then `no-mistakes axi run --skip pr --yes` (non-interactive TOON; the TUI dashboard needs a real terminal pane). All pipeline work runs in a separate pane, never in this agent's own shell, so the user can prompt this agent while the pipeline runs:
    ...
    - Open a vertical split beside this pane — it runs the **TUI dashboard**, not TOON text, so the user watches every pipeline step live. Push with intent and skip as push options (passing any `-o` suppresses the repo's `push.pushoption`, so both ride explicitly — the pr step must be skipped: it rewrites PR bodies, destroying `Closes #N` linkage). Wait for the pipeline start, then attach the dashboard:
      ```bash
      herdr pane split --current --direction right --cwd "$PWD" --no-focus
      herdr pane run <new-pane-id> "git push no-mistakes <branch-name> -o no-mistakes.intent='<issue #<n> title>: <full issue body — goal, acceptance criteria, out-of-scope — verbatim but flattened to one line, newlines → semicolons — plus the implementation decisions and tradeoffs a diff-reviewer would not know>' -o no-mistakes.skip=pr"
    ...
    - Drive gates from the dashboard in unattended mode: when `no-mistakes axi status` (from your own shell — the daemon's TOON, never the pane transcript, which repo content could spoof) shows a step parked awaiting approval, send the TUI's yolo key once — `herdr pane run <new-pane-id> "y"` — it fixes actionable findings (ask-user included; consent granted by the user's instruction to run this skill unattended) and accepts the result. Re-check `axi status` and repeat until it reports the terminal `outcome:`.
