...
2. Run `quota-axi` once before launching. A healthy read is silent; report only a quota finding, a spread/serialize decision for 2+ issues, or a hard-condition ask.
...
- Branch `<project-name>-<N>-<slug>` (kebab-case, 3–5 words). `<project-name>` is the repo name — the segment after `/` in the `nameWithOwner` the precondition's `gh repo view` already printed (e.g. `mguinada/k-wiki` → `k-wiki`, composing `k-wiki-<N>-<slug>`). Snapshot collisions once (`herdr worktree list` + `git branch --list "<project-name>-<N>-*"`); existing → skip and record, never clobber.
