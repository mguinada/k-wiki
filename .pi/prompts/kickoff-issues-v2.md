---
description: Kick off GitHub issue work in parallel — one fresh pi main agent per issue, in its own Herdr worktree, running the do-gh-issue-v2 skill
argument-hint: "[<issue-number> | <issue-URL> | label:<label> | top:<N> by <criterion> ...]"
---

You (host) only resolve issues and launch agents. Each fresh pi main agent implements its issue alone via the `do-gh-issue-v2` skill — the skill already gates on blockers, so the kickoff never re-checks them.
