...
2. **quota pre-flight** — when agent settings name a provider, an
...
3. **`git pull --rebase`** — the run starts on a fresh base; any
...
4. **`wiki-sync`** — unchanged: the full gated cycle ending in a
   ...
   design).
5. **`git push`** — unattended pushing is consented to here and only
