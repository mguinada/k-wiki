...
| `node <checkout>/bin/k-wiki.ts query "<question>"` (also `npm run k-wiki -- query …` inside the checkout) | agent-facing query | Ask the wiki bound to the current project from any cwd — zero flags once `.k-wiki.json` binds it; answer-only, no filing passthrough ([details below](#querying-from-any-project-k-wiki)) |
...
hit this (issue #146). After every agent run three mechanical guardrails check the
...
`--sources <vault/path>` (issue #133) re-ingests exactly the listed
...
1. **sync** — `sync-vault` in-process: vault → `raw/`. A repo-sourced
   ...
   commit (issue #145); a dirty source tree fails the cycle, and
...
`k-wiki` is the agent-facing query entry point (guide §16, issue
#76): a thin wrapper — one `query` command, `util.parseArgs`, no
CLI framework — that resolves the current project's wiki and
delegates to the answer-only stage-1 query above. After #72 the
...
`wiki-query` command inside the checkout. A wrong pairing (a
binding whose checkout resolves an unexpected data repo) fails
loudly via the existing guardrails — no silent cross-wiki reads.
Humans can add a shell alias:
