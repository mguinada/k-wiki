...
| `bin/k-wiki recover-fix-surface [-h \| --help] [show] [<config>] [<raw-dir>]` / `bin/k-wiki recover-fix-surface recover --yes [<config>] [<raw-dir>]` | failed-cycle fix-surface recovery | Recover a failed shared-writer cycle's recorded fix surface: a mid-run failure leaves its dirty paths recorded (paths, content hashes, lease ref + OID + holder + expiry) beside the retained lease; `show` (the default) prints exactly what a recovery would discard and the refused-tick count toward the cycle's own auto-recovery; `recover --yes` discards exactly the recorded paths (tracked revert, staged-add and untracked removal — never a repo-wide reset or clean) and retakes the recorded lease by exact OID under a short recovery lease; it refuses when the live dirty set differs from the record — a human edit made after the failure is never eaten — and works only on a recorded surface: with nothing recorded it does nothing |
...
dead holder. When a cycle fails mid-run, its dirty paths are
recorded with the retained lease, and recovery is one recorded
operation instead of tribal knowledge: `k-wiki recover-fix-surface`
shows the recorded surface, `k-wiki recover-fix-surface recover
--yes` discards exactly the recorded paths and settles the lease
(refusing any human edit the record does not know), and the
schedule itself auto-recovers a still-matching surface after three
consecutive refused ticks once the retained lease has lapsed. Manual shared cycles push by design — enabling the
