...
Initialize Git inside `k-wiki` (the repository root). Keep the checkout in a plain local folder — never inside a cloud-synced folder — and share it between Macs through a private remote (Section 26).
