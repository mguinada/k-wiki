...
### Multi-part source hubs

A multi-part hub — a source page whose note in `raw/` is a directory
of chapters — carries one section per cited chapter, under that
...
a page. A section may legitimately be one line — the anchor is the
...
`- "[[hub-name]]"`, anchored (`- "[[hub#Chapter]]"`) when citing a
sub-source of a multi-part hub: the anchor is the cited chapter's
directory name. Each chapter a hub's own `sources` list cites has one
heading in the hub body, byte-identical to the anchor — generated by
the chapter-anchor migration, never typed free-hand (chapter names
carry irregular whitespace; derived headings cannot drift from the
citations that point at them). When citing material that has no
